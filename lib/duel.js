...
duel.DuelAbstractChannel = (function () {
    ...
    /**
     * Only master can sends trigger
     */
    DuelAbstractChannel.prototype.trigger = function (trigger) {
        if (window.isMaster()) {
            this.executeTrigger({
                name: trigger,
 ...
 * Only master can sends trigger
 */
duel.DuelLocalStorageChannel.prototype.trigger = function (trigger) {
    if (window.isMaster()) {
        console.info('new trigger');
        var triggerDetails = {
            name: trigger,
            args: Array.prototype.slice.call(arguments, 1)
        };
        ...
        //this.executeTrigger(triggerDetails)
    }
};
...
duel.addEvent(window, 'storage', duel.storageEvent);
//TODO: take off hard code and do events if it possible

/*if (duel.isLocalStorageAvailable()) {
    duel.storageOldTriggerValue = '';
    localStorage.setItem('dueljs_trigger', '');
    setInterval(function () {
        if (localStorage.getItem('dueljs_trigger') != duel.storageOldTriggerValue) {
            duel.storageEvent({
                key: 'dueljs_trigger',
